...
class Worker(QObject):
    ...
    def __init__(self, host, port):
        ...
        self._is_muted_client_side = False # New flag for client-side mute
    ...
    def _send_control_event(self, event_type, data):
        """Sends a JSON-formatted control event (mouse, keyboard, mute) to the server."""
    ...
    # New slot to handle mute toggle from the UI
    def toggle_audio_stream(self, is_muted):
        self._is_muted_client_side = is_muted
        self.update_status_signal.emit(f"[*] Audio {'muted' if is_muted else 'unmuted'} client-side.", False)
        # Optionally send a mute command to the server if server-side mute is supported
        self.send_control_event_signal.emit('audio_mute', {'muted': is_muted})
    ...
    def _handle_ffmpeg_stream(self):
        ...
        # Dynamically add '-an' (no audio) if muted client-side
        command = [
        ...
        if self._is_muted_client_side:
            command.append('-an') # Add no-audio flag
        command.append('-i')
        command.append('pipe:0')
    ...
    def _handle_legacy_stream(self):
        ...
        if PYAUDIO_SUPPORT:
            # Pass the mute state to the audio thread
    ...
    def _play_legacy_audio(self):
        ...
        try:
            while not self.stop_event.is_set():
                ...
                if not self._is_muted_client_side: # Only play audio if not muted
...
class ClientWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.is_muted = False # New state for mute button
    ...
    def _create_utility_buttons(self, layout):
        ...
        self.about_button = QPushButton("ⓘ About") # New About button
        self.mute_button = QPushButton("🔇 Mute") # New Mute button
        ...
        utility_buttons = [
            self.fullscreen_button, self.logs_button, self.about_button, self.mute_button, self.exit_button
        ]
    ...
    def _setup_connections(self):
        ...
        self.about_button.clicked.connect(self.open_about_link) # Connect About button
        self.mute_button.clicked.connect(self.toggle_mute) # Connect Mute button
    ...
    def open_about_link(self):
        """Opens the specified GitHub link in the default web browser."""
        webbrowser.open("https://github.com/Nakildias/PyViewer")
        self.update_status("[*] Opened PyViewer GitHub page in browser.", False)

    def toggle_mute(self):
        """Toggles the mute state and updates the button text and worker."""
        self.is_muted = not self.is_muted
        if self.is_muted:
            self.mute_button.setText("🔊 Unmute")
            self.update_status("[*] Client audio muted.", False)
        else:
            self.mute_button.setText("🔇 Mute")
            self.update_status("[*] Client audio unmuted.", False)

        if self.worker:
            # Signal the worker to handle the mute state
            self.worker.toggle_audio_stream(self.is_muted)
        else:
            self.update_status("[!] Not connected, cannot toggle audio.", True)
